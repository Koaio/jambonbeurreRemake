...
RegisterServerEvent('koaioezdynastyy:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es')
AddEventHandler('koaioezdynastyy:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es', function(reason,servertarget)
	...
	if target and target > 0 then
		...
		if ping and ping > 0 then
			if duree and duree < 365 then
				local sourceplayername = "Dynasty"
...
--calcul total expiration (en secondes)
	...
	MySQL.Async.execute('INSERT INTO tismey_ban (license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,reason,expiration,timeat,permanent) VALUES (@license,@identifier,@liveid,@xblid,@discord,@playerip,@targetplayername,@sourceplayername,@reason,@expiration,@timeat,@permanent)', {
		['@license']          = license,
		['@identifier']       = identifier,
		['@liveid']           = liveid,
		['@xblid']            = xblid,
...
function loadBanList()
	MySQL.Async.fetchAll('SELECT * FROM tismey_ban', {}, function(data)
		BanList = {}
